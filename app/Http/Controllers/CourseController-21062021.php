...
class CourseController extends Controller
{
    ...
    public function  showCourse($slug)
    {
        try {
            ...
            return view('course.show-course', compact('course', 'title', 'total_en', 'desc', 'c_img','rating_avg',
                    'rated_by_students'            
        ));
        } catch (\Throwable $th) {
            return back()->with('error', 'internal server erro');
        }
    }
    ...
    public function showAllCourses()
    {
        try {
            $title = 'Courses';
            $courses = Course::with(['course_image', 'price', 'user'])->where('status', 'published')->select(
                'id',
                'user_id',
                'course_title',
                'categories_selection',
